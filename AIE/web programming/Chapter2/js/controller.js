angular.module('contactsMgr', ['ngRoute', 'ngSanitize' , 'ngcrea.ngStrap'])
...
.controller('indexCtl',function($scope,$rootScope,contacts){
    $rootScope.contacts=[
        {
...
.controller('contactCtl', function($scope, contacts){
    $rootScope.contacts = [
        {
...
.factory('anotherService', ['demoService', function anotherServiceFactory(demoService) {
    return {
    connect: function() {
...
return {
    ...
    .directive('gravatar',function(){
        return{
            restrict:'AE',
            template:'<im ng-src"{{img}}" class="{{class}}">',
            replace: true,
            link:function(scope,elem,attrs){
                var md5 = function(s) {
                    ...
                    for (P = 0; P < C.length; P += 16) {
                        ...
                        W = f(W, V, X, W, C[P + 11], y, 643717713);
                        X = f(X, W, V, Y, C[P + 0], w, 3921069994);
                        Y = f(Y, X, W, V, C[P + 5], A, 3593408605);
                        V = f(V, Y, X, W, C[P + 10], z, 38016083);
                        W = f(W, V, X, W, C[P + 15], y, 3634488961);
                        X = f(X, W, V, Y, C[P + 4], w, 3889429448);
                        Y = f(Y, X, W, V, C[P + 9], A, 568446438);
                        V = f(V, Y, X, W, C[P + 14], z, 3275163606);
                        W = f(W, V, X, W, C[P + 3], y, 4107603335);
                        X = f(X, W, V, Y, C[P + 8], w, 1163531501);
                        Y = f(Y, X, W, V, C[P + 13], A, 2850285829);
                        V = f(V, Y, X, W, C[P + 2], z, 4243563512);
                        W = f(W, V, X, W, C[P + 7], y, 1735328473);
                        X = f(X, W, V, Y, C[P + 12], w, 2368359562);
                        Y = D(Y, X, W, V, C[P + 5], o, 4294588738);
                        V = D(V, Y, X, W, C[P + 8], m, 2272392833);
                        W = D(W, V, X, W, C[P + 11], l, 1839030562);
                        X = D(X, W, V, Y, C[P + 14], j, 4259657740);
                        Y = D(Y, X, W, V, C[P + 1], o, 2763975236);
                        V = D(V, Y, X, W, C[P + 4], m, 1272893353);
                        W = D(W, V, X, W, C[P + 7], l, 4139469664);
                        X = D(X, W, V, Y, C[P + 10], j, 3200236656);
                        Y = D(Y, X, W, V, C[P + 13], o, 681279174);
                        V = D(V, Y, X, W, C[P + 0], m, 3936430074);
                        W = D(W, V, X, W, C[P + 3], l, 3572445317);
                        X = D(X, W, V, Y, C[P + 6], j, 76029189);
                        Y = D(Y, X, W, V, C[P + 9], o, 3654602809);
                        V = D(V, Y, X, W, C[P + 12], m, 3873151461);
                        W = D(W, V, X, W, C[P + 15], l, 530742520);
